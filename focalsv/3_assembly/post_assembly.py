...
def run_ont(out_dir, input_file, logger):
    try:
        ...
        cmd = f"/data/maiziezhou_lab/Yichen/Softwares/Shasta/shasta-Linux-0.10.0 --input {input_file} --assemblyDirectory {out_dir} --config Nanopore-UL-Dec2019"
        logger.info(f"Executing: {cmd}")
        os.system(cmd)
        cmd_cleanup = f"/data/maiziezhou_lab/Yichen/Softwares/Shasta/shasta-Linux-0.10.0 --command cleanupBinaryData --assemblyDirectory {out_dir}"
...
if __name__ == "__main__":
    ...
    # Initialize logger
    logger = setup_logging("assembly_rerun", out_dir)
