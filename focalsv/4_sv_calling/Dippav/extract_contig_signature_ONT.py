...
#         print("judge")
        ...
        if abs(Diffdis)<=max_svlen:
            ...
            #     if -(Diffdis*r) <= Diffolp <= (Diffdis*r) :
            #         sigins = [read1.reference_name,'INS', int((Ref1e+Ref2s)/2), abs(Read2s-Read1e+Diffolp) , read1.qname,Read1e-Diffolp, Read2s,direction,'split-alignment',"%d-%d"%(read1.mapq,read2.mapq)]
            ...
            #     del_list.append(sigdel)
            elif ( Diffdis*0.8 <= Diffolp <= abs(Diffdis)*r ) and (Diffdis<= -30) :
...
def write_vcf(paired_sig_sorted,vcf_path,ref_path,contig_path,header_path):
...
def extract_contig_sig_ONT(chr_number,bam_path,header_path,
